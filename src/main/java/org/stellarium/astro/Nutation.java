...
class Nutation {
    public static final int TERMS = 63;
    public static final double LN_NUTATION_EPOCH_THRESHOLD = 0.1;

    /**
     * arguments and coefficients taken from table 21A on page 133
     */
    public static class lnNutation {
    ...
    static public class nutationCoefficients {
    ...
    final static nutationArguments[] arguments = {
    ...
    final static nutationCoefficients[] coefficients = {
            ...
            new nutationCoefficients(63.0, 1.0, -33.0, 0.0),
     ...
     *
     * @param JD Julian Day
     * @return Stored nutation.
     *         Chapter 21 pg 131-134 Using Table 21A
     */
    static lnNutation getNutation(double JD) {
        ...
        if (Math.abs(JD - c_JD) > LN_NUTATION_EPOCH_THRESHOLD) {
            /* set the new epoch */
            c_JD = JD;
            ...
            for (i = 0; i < TERMS; i++) {
                ...
                /* sum the arguments */
                if (arguments[i].D != 0) {
                    c_longitude += coeff_sine * (Math.sin(arguments[i].D * D));
                    c_obliquity += coeff_cos * (Math.cos(arguments[i].D * D));
                }
                if (arguments[i].M != 0) {
                    c_longitude += coeff_sine * (Math.sin(arguments[i].M * M));
                    c_obliquity += coeff_cos * (Math.cos(arguments[i].M * M));
                }
                if (arguments[i].MM != 0) {
                    c_longitude += coeff_sine * (Math.sin(arguments[i].MM * MM));
                    c_obliquity += coeff_cos * (Math.cos(arguments[i].MM * MM));
                }
                if (arguments[i].F != 0) {
                    c_longitude += coeff_sine * (Math.sin(arguments[i].F * F));
                    c_obliquity += coeff_cos * (Math.cos(arguments[i].F * F));
                }
                if (arguments[i].O != 0) {
                    c_longitude += coeff_sine * (Math.sin(arguments[i].O * O));
                    c_obliquity += coeff_cos * (Math.cos(arguments[i].O * O));
                }
            }
